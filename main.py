...
class subwindow(QtWidgets.QWidget):
    def __init__(self):
        ...
        self.ui2.label.setText(f'<html><head/><body><p>接下來將會用您的預設瀏覽器開啟所有的{video_num}部影片，</p><p>您確定要繼續嗎</p></body></html>')
...
def main(playlist):
    ...
    if len(str(playlist)) == 34 or len(str(playlist)) == 13:  # 檢查所輸入之字元是否為播放清單或youtube合輯的ID
        playlistid = playlist
    elif 'http' not in playlist or 'list' not in playlist or str(playlist) == '':  # 檢查所輸入之字元是否為網址 以及是否為單一影片的網址
        print('錯誤')
    else:
